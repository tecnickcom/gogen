...
func Test_Exec_RollbackAfterFailedCommit(t *testing.T) {
	...
	err = Exec(t.Context(), db, func(_ context.Context, _ *sqlx.Tx) error {
		return nil
	})
	require.Error(t, err)
	require.ErrorIs(t, err, commitErr)
	require.Contains(t, err.Error(), "unable to commit SQLX transaction")
	require.NotContains(t, err.Error(), "failed rolling back SQLX transaction")
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func Test_Exec_PanicRollsBackAndPropagates(t *testing.T) {
	...
	func() {
		...
		_ = Exec(t.Context(), db, func(_ context.Context, _ *sqlx.Tx) error {
			panic(panicValue)
		})
	}()
...
func Test_ExecWithOptions_NilDB(t *testing.T) {
	...
	err := Exec(t.Context(), nil, func(_ context.Context, _ *sqlx.Tx) error { return nil })
